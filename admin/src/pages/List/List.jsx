    ...
    try {
      ...
      if (response.data.success) {
        setList(prev =>
          prev.map(item =>
            item._id === foodId
              ? { ...item, price: type === "add" ? item.price + 1 : item.price - 1 }
              : item
          )
        )
      } else {
  ...
  return (
    <div className='list-add'>
      ...
      <div className="list-table">
        ...
        {list.length === 0 && (
          <p style={{ padding: '20px', textAlign: 'center' }}>No food items found</p>
        )}
        ...
        {list.map(item => (
          <div key={item._id} className="list-table-format">
            ...
            <div className="price-controls">
              <span className="price-btn" onClick={() => changePrice(item._id, "sub")} aria-label="Decrease Price">-</span>
              <p>₹{item.price}</p>
              <span className="price-btn" onClick={() => changePrice(item._id, "add")} aria-label="Increase Price">+</span>
            </div>

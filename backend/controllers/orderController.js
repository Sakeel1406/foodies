...
export const placeOrder = async (req, res) => {
    try {
        ...
        const discountAmount = discountPercent > 0 ? (amount * discountPercent) / 100 : 0;
        ...
        const sessionOptions = {
            ...
            metadata: { userId: userId.toString(), orderId: newOrder._id.toString() }
        };
    ...
    try {
        ...
        if (success === "true" || success === true) {
            ...
            if (session.payment_status === "paid") {
                ...
                const user = await userModel.findByIdAndUpdate(session.metadata.userId, { cartData: {} }, { new: true });

                const mailOptions = {
                    from: process.env.EMAIL_USER,
                    to: user.email, 
                    subject: `Order Confirmed! - #${orderId.slice(-6).toUpperCase()}`,
                    html: `<h2>Order Confirmed</h2><p>Hi ${user.name}, payment of ₹${updatedOrder.amount} successful.</p>`
                };
                transporter.sendMail(mailOptions);

                return res.json({ success: true, message: "Verified & Receipt Sent" });
            }
        }
...
export const addFeedback = async (req, res) => {
    try {
        ...
        const updatedOrder = await orderModel.findByIdAndUpdate(orderId, { feedback: feedback }, { new: true });
